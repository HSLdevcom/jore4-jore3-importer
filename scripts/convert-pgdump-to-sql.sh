...
TARGET_DIR="$(dirname "$(dirname "$(dirname "$(realpath "${BASH_SOURCE[0]}")")")")/jore-data/database_dumps"
SUFFIX=""
...
print_usage() {
  ...
  echo "  --suffix, -s         Suffix to append to database role names (optional, dev | test, default: empty)"
...
FILE_BASENAME=$(date "+%Y-%m-%d")-jore4-local-"$DATABASE"
