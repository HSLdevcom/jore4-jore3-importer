...
if [ -z "${SOURCE_DB_URL+x}" ]; then
  export SOURCE_DB_URL="jdbc:sqlserver://${SOURCE_DB_HOSTNAME}:${SOURCE_DB_PORT};database=${SOURCE_DB_DATABASE};applicationIntent=ReadOnly;encrypt=false;trustServerCertificate=true"
fi
