...
| importer.db.url         | IMPORTER_DB_URL         | importer-db-url         | jdbc:postgresql://localhost:5432/devdb?stringtype=unspecified                    | The jdbc url of the destination JORE4 PostgreSQL database                        |
|                         | IMPORTER_DB_HOSTNAME    | importer-db-hostname    | localhost                                                                        | The IP/hostname of the destination database (if DESTINATION_DB_URL is not set)   |
|                         | IMPORTER_DB_PORT        | importer-db-port        | 5432                                                                             | The port of the destination database (if DESTINATION_DB_URL is not set)          |
|                         | IMPORTER_DB_DATABASE    | importer-db-database    | devdb                                                                            | The name of the destination database (if DESTINATION_DB_URL is not set)          |
| importer.db.username    | IMPORTER_DB_USERNAME    | importer-db-username    | postgres                                                                         | Username for the destination database                                            |
| importer.db.password    | IMPORTER_DB_PASSWORD    | importer-db-password    | ****                                                                             | Password for the destination database                                            |
