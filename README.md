...
value will reflect the status of the running job.
...
The API endpoint returns the status of the latest import job. If no import job has been run, the API endpoint returns
...
This application is basically a Spring Boot application which imports the data from the Jore 3 database to the Jore 4
database by using Spring Batch. This application also provides a REST API which allows you to start the import job and 
...
* The `exportJourneyPatternsStep` step creates new journey patterns to the Jore 4 database.
* The `exportJourneyPatternStopsStep` step imports links scheduled stop points with the created journey patterns.
