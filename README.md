...
* When you add new fields to entities or data transfer objects, you must follow these
  rules:
  * If the field value cannot be `null`, you must use primitive types when possible (e.g. `int`). If the field contains
    a string, set its type to `String`.
  * If the field value is optional, you must use `java.util.Optional` (e.g. `Optional<Integer>` or `Optional<String>`)
* Tag nullable method parameters and return values with the `@Nullable` annotation. Non-null parameters and return 
  values should not be annotated because they are already covered by `package-info.java`.
