 ...
 * <ul>
 *     <li>Postgresql {@code tstzrange} values in the ISO 8601 format (e.g. {@code "[2004-10-19 10:23:54+02,2005-10-19 10:23:54+02)")} and
...
public class TimeRangeConverter implements Converter<Object, TimeRange> {
    ...
    /**
     * We cant use {@link java.time.format.DateTimeFormatter#ISO_LOCAL_DATE_TIME ISO_LOCAL_DATE_TIME} directly,
     * as "ISO 8601 specifies the use of uppercase letter T to separate the date and time. PostgreSQL accepts
     * that format on input, but on output it uses a space rather than T."
     */
    ...
    public TimeRange from(@Nullable final Object t) {
        ...
        if (m.find()) {
            ...
            return TimeRangeUtil.asTimeRange(from,
                                             to,
                                             startInclusive,
                                             endInclusive);
        } else {
