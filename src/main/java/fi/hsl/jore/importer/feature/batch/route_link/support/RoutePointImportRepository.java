...
public class RoutePointImportRepository
        ...
        return db
                ...
                .whereNotExists(
                        selectOne()
                                .from(STAGING_TABLE)
                                .where(
                                        STAGING_TABLE.NETWORK_ROUTE_POINT_EXT_ID.eq(
    ...
    protected Set<RoutePointPK> update() {
        return db
                ...
                // Find source rows..
                .where(
                        TARGET_TABLE.NETWORK_ROUTE_POINT_EXT_ID.eq(
                                STAGING_TABLE.NETWORK_ROUTE_POINT_EXT_ID))
                // .. with updated fields
                .and(
                        (TARGET_TABLE.NETWORK_ROUTE_POINT_ORDER.ne(
                                STAGING_TABLE.NETWORK_ROUTE_POINT_ORDER)))
                .returningResult(TARGET_TABLE.NETWORK_ROUTE_POINT_ID)
                .fetch()
                .stream()
        ...
        return db
                ...
                .select(
                        db.select(
                                ...
                                .on(
                                        ROUTE_DIRECTIONS_TABLE.NETWORK_ROUTE_DIRECTION_EXT_ID.eq(
                                                STAGING_TABLE.NETWORK_ROUTE_DIRECTION_EXT_ID))
                                // At the moment there are about 150 route points, which are
                                // discarded because they
                                // refer to non-existing nodes
                                .innerJoin(NODES_TABLE)
                                .on(
                                        NODES_TABLE.INFRASTRUCTURE_NODE_EXT_ID.eq(
                                                STAGING_TABLE.INFRASTRUCTURE_NODE_EXT_ID))
                                .whereNotExists(
                                        selectOne()
                                                .from(TARGET_TABLE)
