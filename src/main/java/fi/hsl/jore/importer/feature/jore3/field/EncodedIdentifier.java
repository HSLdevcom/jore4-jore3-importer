 ...
 *         <li>Deprecated in favor of {@link
 *             fi.hsl.jore.importer.feature.jore3.enumerated.PublicTransportDestination} -enum in
 *             the line/route tables
 *       </ul>
 *   <li>B: Line number
 *       <ul>
 ...
 * </ul>
 *
 * <p>The actual line number shown to the passenger is constructed from the B, C and D fields. This
 * passenger-facing line number is <i>not</i> unique!
 *
 * <p>See: <a href="https://github.com/HSLdevcom/jore4/blob/main/wiki/jore3_route_ids.md">this</a>.
 *
 * <p>This class uses the {@link org.immutables.value.Value.Lazy Lazy} annotation to evaluate (and
 * cache) the individual fields only when requested instead of eagerly evaluating them. This
 * improves performance in situations where we are dealing with large amounts of identifiers but
 * only care about the {@link EncodedIdentifier#originalValue() original value}.
 */
public abstract class EncodedIdentifier {
    ...
    /**
     * For train, second and third are zeroes and fourth is 1 or 2. Two is for trains operating west
     * from Pasila and one for ones operating north/east.
     *
    ...
    /**
     * For metro, second is one and third and fourth are the displayed line number, e.g. "31M2" ->
     * line number is "1M2" -> displayed identifier is "M2".
     *
    ...
    /**
     * Check if this identifier represents a hidden variant. Hidden variants are used for routes
     * which differ only very slightly from the main route and we don't need to indicate to the
     * passenger that this route is a variant.
     */
    ...
    /**
     * Some lines/routes are not (or never were) actually operational. Most of the time they are
     * identified with "x000", for example "1000", "1000 X". These lines/routes don't always produce
     * sane visible lane numbers for the passenger, e.g. "1000" -> "", but that's not a problem as
     * these are not operational.
     *
    ...
    public Optional<Short> hiddenVariantValue() {
        // The hidden variant type in Jore 4 DB is nullable smallint, so return it in matching
        // format already.
        if (hiddenVariantB()) {
