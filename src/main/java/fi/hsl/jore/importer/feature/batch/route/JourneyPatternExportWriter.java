...
public class JourneyPatternExportWriter implements ItemWriter<Jore4JourneyPattern> {
    ...
    public void write(final Chunk<? extends Jore4JourneyPattern> items) throws Exception {
        ...
        importerRepository.setJourneyPatternJore4Ids(FluentIterable.from(items)
                .transform(item ->
                        PersistableJourneyPatternIdMapping.of(item.routeDirectionExtId(), item.journeyPatternId())));
    }
}
