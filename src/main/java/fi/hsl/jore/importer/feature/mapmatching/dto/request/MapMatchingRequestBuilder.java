...
public class MapMatchingRequestBuilder {
    ...
    /**
     * Creates a new map matching request which can be send to the map matching API.
     *
    ...
    public static MapMatchingRequestDTO createMapMatchingRequest(
            final ImporterRouteGeometry routeGeometry,
    ...
    private static List<RoutePointRequestDTO> createRequestRoutePoints(
            final io.vavr.collection.List<ImporterRoutePoint> inputRoutePoints) {
        final List<RoutePointRequestDTO> routePoints = new ArrayList<>();

        inputRoutePoints.forEach(
                input -> {
                    ...
                    if (input.type() == NodeType.STOP) {
                        ...
                        routePoint.setPassengerId(
                                input.stopPointShortId()
                                        .orElseThrow(
                                                () ->
                                                        new NullPointerException(
                                                                String.format(
                                                                        "Cannot create map matching"
                                                                            + " request."
                                                                            + " passengerId is null"
                                                                            + " and the route point"
                                                                            + " is stop point with"
