...
/**
 * JrNodes don't know their network (e.g. bus/metro/train), instead this information is stored in
 * the JrLink between nodes. Downstream consumers most likely want to assign nodes to networks
 */
@JoreDtoStyle
@Value.Immutable
public interface LinkRow {
    ...
    default void checkLinkContext() {
        ...
        Preconditions.checkState(
                from().location().getSRID() == to().location().getSRID(),
