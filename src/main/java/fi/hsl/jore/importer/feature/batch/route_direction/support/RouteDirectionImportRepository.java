...
public class RouteDirectionImportRepository
    ...
    private static final NetworkRouteDirections TARGET_TABLE =
        ...
        return db
                ...
                .whereNotExists(
                        selectOne()
                                .from(STAGING_TABLE)
                                .where(
                                        STAGING_TABLE.NETWORK_ROUTE_DIRECTION_EXT_ID.eq(
    ...
    protected Set<RouteDirectionPK> update() {
        return db
                ...
                // Find source rows..
                .where(
                        TARGET_TABLE.NETWORK_ROUTE_DIRECTION_EXT_ID.eq(
                                STAGING_TABLE.NETWORK_ROUTE_DIRECTION_EXT_ID))
                // .. with updated fields
                .and(
                        (TARGET_TABLE.NETWORK_ROUTE_DIRECTION_LENGTH.ne(
                                        STAGING_TABLE.NETWORK_ROUTE_DIRECTION_LENGTH))
                                .or(
                                        TARGET_TABLE.NETWORK_ROUTE_DIRECTION_NAME.ne(
                                                STAGING_TABLE.NETWORK_ROUTE_DIRECTION_NAME))
                                .or(
                                        TARGET_TABLE.NETWORK_ROUTE_DIRECTION_NAME_SHORT.ne(
                                                STAGING_TABLE.NETWORK_ROUTE_DIRECTION_NAME_SHORT))
                                .or(
                                        TARGET_TABLE.NETWORK_ROUTE_DIRECTION_ORIGIN.ne(
                                                STAGING_TABLE.NETWORK_ROUTE_DIRECTION_ORIGIN))
                                .or(
                                        TARGET_TABLE.NETWORK_ROUTE_DIRECTION_DESTINATION.ne(
                                                STAGING_TABLE.NETWORK_ROUTE_DIRECTION_DESTINATION))
                                .or(
                                        TARGET_TABLE.NETWORK_ROUTE_DIRECTION_VALID_DATE_RANGE.ne(
                                                STAGING_TABLE
        ...
        return db
                ...
                .select(
                        db.select(
                                ...
                                .on(
                                        ROUTES_TABLE.NETWORK_ROUTE_EXT_ID.eq(
                                                STAGING_TABLE.NETWORK_ROUTE_EXT_ID))
                                .whereNotExists(
                                        selectOne()
                                                .from(TARGET_TABLE)
    ...
    @Transactional
    @Override
    public void setJourneyPatternJore4Ids(
            final Iterable<PersistableJourneyPatternIdMapping> idMappings) {
        db.batched(
                c -> {
                    idMappings.forEach(
                            idMapping -> {
                                c.dsl()
                                        .update(TARGET_TABLE)
                                        .set(
                                                TARGET_TABLE.JOURNEY_PATTERN_JORE4_ID,
                                                idMapping.journeyPatternId())
                                        .where(
                                                TARGET_TABLE.NETWORK_ROUTE_DIRECTION_ID.eq(
                                                        idMapping.routeDirectionId()))
                                        .execute();
                            });
                });
    }
                    ...
                    idMappings.forEach(
                            idMapping -> {
                                c.dsl()
                                        .update(TARGET_TABLE)
                                        .set(
                                                TARGET_TABLE.NETWORK_ROUTE_JORE4_ID,
                                                idMapping.jore4Id())
                                        .where(
                                                TARGET_TABLE.NETWORK_ROUTE_DIRECTION_ID.eq(
                                                        idMapping.routeDirectionId()))
