...
@Component
public class MapMatchingProcessor
        implements ItemProcessor<ImporterRouteGeometry, Jore4RouteGeometry> {
    ...
    @Override
    public Jore4RouteGeometry process(final ImporterRouteGeometry routeGeometryInput)
            throws Exception {
        LOGGER.debug(
        ...
        final List<ImporterRoutePoint> routePointsInput =
                routePointRepository.findImporterRoutePointsByRouteDirectionId(
                        routeGeometryInput.routeDirectionId());
        if (routePointsInput.isEmpty()) {
            LOGGER.error(
                    "Cannot process route geometry because no route points were found for route"
                            + " with route direction id: {}",
                    routeGeometryInput.routeDirectionId());
            return null;
        }
