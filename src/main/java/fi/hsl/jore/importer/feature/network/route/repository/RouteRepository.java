...
@Repository
public class RouteRepository implements IRouteTestRepository {
    ...
    @Autowired
    public RouteRepository(@Qualifier("importerDsl") final DSLContext db, final IJsonbConverter jsonbConverter) {
        this.db = db;
    ...
    public RoutePK insert(final PersistableRoute route) {
        ...
        r.setNetworkRouteLegacyHslMunicipalityCode(
    ...
    public RoutePK update(final Route route) {
        final NetworkRoutesRecord r = Optional.ofNullable(db.selectFrom(ROUTE)
    ...
    public int count() {
        ...
        return db.selectCount().from(ROUTE).fetchOne(0, int.class);
    }
