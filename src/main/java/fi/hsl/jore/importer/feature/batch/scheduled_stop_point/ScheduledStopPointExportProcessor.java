...
/**
 * Combines the stop information imported from Jore 3 and Digiroad, and creates a new {@link
 * Jore4ScheduledStopPoint} object which can be inserted into the Jore 4 database.
 */
@Component
public class ScheduledStopPointExportProcessor
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(ScheduledStopPointExportProcessor.class);
    ...
    @Override
    public Jore4ScheduledStopPoint process(final ImporterScheduledStopPoint importerStop)
            throws Exception {
        ...
        if (externalIds.size() != elyNumbers.size()) {
            LOGGER.debug(
                    "Error processing the the stop with short ID: {}. The amount of external IDs {}"
                        + " differs from the amount of ELY numbers {} which blocks processing any"
                        + " further.",
        ...
        for (int index = 0; index < elyNumbers.size(); index++) {
            ...
            final Optional<DigiroadStop> digiroadStopContainer =
                    digiroadStopService.findByNationalId(elyNumber);
            if (digiroadStopContainer.isPresent()) {
                ...
                final Jore4ScheduledStopPoint jore4Stop =
        ...
        LOGGER.error(
                "Stop point with short ID: {} isn't processed any further no digiroad stop was"
                        + " found with national ids: {}",
                importerStop.shortId().get(),
                elyNumbers);
        return null;
    }
