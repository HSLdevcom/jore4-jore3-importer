...
/**
 * Cleans up the Jore 4 database so that new scheduled stop points can be
 * inserted into the <code>internal_service_pattern.scheduled_stop_point</code>
 * database table found from the Jore 4 database.
 */
@Component
public class TransmodelScheduledStopPointCleanupTasklet implements Tasklet  {

    private final DSLContext db;

    @Autowired
    public TransmodelScheduledStopPointCleanupTasklet(@Qualifier("jore4Dsl") DSLContext db) {
        this.db = db;
    }
