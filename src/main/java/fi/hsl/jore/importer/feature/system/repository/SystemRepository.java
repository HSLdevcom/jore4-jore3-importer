...
public class SystemRepository implements ISystemRepository {
    ...
    public Instant currentTimestamp() {
        return db.select(DSL.currentInstant()).fetchOne().value1();
    }

    @Override
    public LocalDate currentDate() {
        return db.select(DSL.currentLocalDate()).fetchOne().value1();
    }
    ...
    public LocalTime currentTime() {
        ...
        return db.select(DSL.field("LOCALTIME", Time.class)).fetchOne().value1().toLocalTime();
    }
}
