...
/**
 * Transforms the information of a journey pattern stop point into a format that can be inserted to
 * the Jore 4 database.
 */
@Component
public class JourneyPatternStopExportProcessor
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(JourneyPatternStopExportProcessor.class);

    @Override
    public Jore4JourneyPatternStop process(final ImporterJourneyPatternStop input)
            throws Exception {
    ...
    private static boolean isUsedAsTimingPoint(final ImporterJourneyPatternStop stop) {
        ...
        if (stop.isUsedAsTimingPoint()) {
            if (stop.timingPlaceLabel().isPresent()) {
                ...
                LOGGER.warn(
                        "Jore3 route direction {}: route point {}: stop point marked as a timing"
                                + " point but no timing place label present => setting"
                                + " isUsedAsTimingPoint flag to false",
    ...
    private static boolean isRegulatedTimingPoint(
        ...
        if (stop.regulatedTimingPointStatus().isRegulatedTimingPoint()) {
            if (isUsedAsTimingPoint) {
                ...
                LOGGER.warn(
                        "Jore3 route direction {}: route point {}: stop point marked as a regulated"
                            + " timing point, but at the same time it is not used as a timing point"
                            + " => discarding regulated timing point status",
                        stop.routeDirectionJore3Id(),
                        stop.orderNumber());
            }
        }
    ...
    private static boolean isLoadingTimeAllowed(
        ...
        return isUsedAsTimingPoint
                && stop.regulatedTimingPointStatus() == RegulatedTimingPointStatus.YES_LOAD_TIME;
    }
}
