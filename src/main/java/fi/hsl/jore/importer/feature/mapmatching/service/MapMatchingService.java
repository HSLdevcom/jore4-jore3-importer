...
public class MapMatchingService implements IMapMatchingService {
    ...
    public MapMatchingService(
            final String mapMatchingApiBaseUrl,
            final ObjectMapper objectMapper,
            final RestTemplate restTemplate) {
        this.mapMatchingApiUrl =
                MapMatchingApiUrlFactory.buildMapMatchingApiUrl(mapMatchingApiBaseUrl);
    ...
    public MapMatchingSuccessResponseDTO sendMapMatchingRequest(
        ...
        LOGGER.debug(
                "Sending map matching request for the route direction: {}",
        ...
        try {
            ...
            LOGGER.debug(
                    "Received map matching response for the route direction: {}",
                    routeGeometry.routeDirectionId());
            return mapMatchingResponse;
        } catch (final Exception ex) {
            final String errorMessage =
                    String.format(
                            "Map matching failed for the route direction: %s because of an error:"
                                    + " %s",
    ...
    private MapMatchingSuccessResponseDTO getSuccessResponse(
            final ResponseEntity<String> httpResponse) throws JsonProcessingException {
