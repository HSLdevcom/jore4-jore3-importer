...
public class PlaceImportRepository extends AbstractImportRepository<PersistablePlace, PlacePK>
    ...
    private static final NetworkPlacesStaging STAGING_TABLE =
                        ...
                        db.select(
                                ...
                                .whereNotExists(
                                        selectOne()
                                                .from(TARGET_TABLE)
                                                .where(
                                                        TARGET_TABLE.NETWORK_PLACE_EXT_ID.eq(
                                                                STAGING_TABLE
        ...
        return db
                ...
                .whereNotExists(
                        selectOne()
                                .from(STAGING_TABLE)
                                .where(
                                        STAGING_TABLE.NETWORK_PLACE_EXT_ID.eq(
                                                TARGET_TABLE.NETWORK_PLACE_EXT_ID)))
                .returningResult(TARGET_TABLE.NETWORK_PLACE_ID)
                .fetch()
                .stream()
                .map(row -> PlacePK.of(row.value1()))
