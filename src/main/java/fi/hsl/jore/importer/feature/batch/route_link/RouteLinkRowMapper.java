...
public class RouteLinkRowMapper implements RowMapper<SingleRouteLinkAndParent> {
    ...
    @Override
    @Nullable
    public SingleRouteLinkAndParent mapRow(final ResultSet rs, final int rowNum)
            throws SQLException {
        return SingleRouteLinkAndParent.of(
                ...
                LastLinkAttributes.of(
                        // Note that this field is different for each row, as each row refers to a
                        // different "loppusolmu"!
                        NodeType.of(getStringOrThrow(rs, "loppusolmu_tyyppi"))
                                .orElse(NodeType.UNKNOWN),
                        // Note that these fields are duplicated for each route link as they refer
                        // to the same parent route direction!
