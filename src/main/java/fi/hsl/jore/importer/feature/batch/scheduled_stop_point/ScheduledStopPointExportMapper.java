...
public class ScheduledStopPointExportMapper implements RowMapper<ImporterScheduledStopPoint> {
    ...
    private static final GeometryConverter POINT_CONVERTER =
    ...
    @Override
    public ImporterScheduledStopPoint mapRow(final ResultSet resultSet, final int rowNumber)
            throws SQLException {
    ...
    private Point pointFromDatabaseObject(@Nullable final Object databaseObject)
            throws SQLException {
        if (databaseObject == null) {
            throw new SQLException(
                    "Cannot parse location because the result set contained null value");
        }
