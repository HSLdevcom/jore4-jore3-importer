...
class DigiroadStopFactory {
    ...
    /**
     * Parses the information of a {@link DigiroadStop} object from line which is read from a CSV
     * file. The file must use a semicolon (;) as a separator character. The line must contain the
        ...
        if (columns.length != VALID_LINE_COLUMN_COUNT) {
            throw new IllegalArgumentException(
                    String.format(
                            ...
                            VALID_LINE_COLUMN_COUNT, columns.length));
        }
    ...
    private static Point parseLocation(final String value) {
        try {
            final org.geojson.Point point =
                    (org.geojson.Point)
                            OBJECT_MAPPER.readValue(
                                    removeExtraQuotationMarks(value.trim()), GeoJsonObject.class);
            final LngLatAlt coordinates = point.getCoordinates();
            return JoreGeometryUtil.fromDbCoordinates(
                    coordinates.getLatitude(), coordinates.getLongitude());
        } catch (final JsonProcessingException ex) {
            throw new RuntimeException(
                    String.format("Couldn't parse location from value: %s", value), ex);
        }
    }

    /**
     * Transforms the value: "{""type"": ""Point"", ""coordinates"": [24.696376131, 60.207149801]}"
     * to value: {"type": "Point", "coordinates": [24.696376131, 60.207149801]}
     */
