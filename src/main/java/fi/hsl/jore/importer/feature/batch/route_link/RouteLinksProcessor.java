 ...
 * within a {@link RouteLinksAndAttributes container}), construct list of route {@link Jore3RoutePoint points} and route
 * {@link Jore3RouteStopPoint stop points} (embedded into a {@link Jore3RoutePointsAndLinks container}).
 *
 * <p>Note that it's not guaranteed that a route direction starts and ends in a bus stop! This restriction applies to
 * current route directions, but historic route directions might start or end with e.g. a cross roads junction. Ossi
 * Berg suggested that such leading or trailing non-bus-stop points could be filtered out, however the current
 * implementation does not implement such filtering and instead preserves the original structure.
 */
public class RouteLinksProcessor implements ItemProcessor<RouteLinksAndAttributes, Jore3RoutePointsAndLinks> {
