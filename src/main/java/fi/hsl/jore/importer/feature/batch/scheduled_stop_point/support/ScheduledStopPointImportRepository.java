...
public class ScheduledStopPointImportRepository
    ...
    private static final ScheduledStopPoints TARGET_TABLE =
            ScheduledStopPoints.SCHEDULED_STOP_POINTS;

    private final DSLContext db;
    private final IJsonbConverter jsonbConverter;

    public ScheduledStopPointImportRepository(
        ...
        return db
                ...
                .whereNotExists(
                        db.selectOne()
                                .from(STAGING_TABLE)
                                .where(
                                        STAGING_TABLE.SCHEDULED_STOP_POINT_EXT_ID.eq(
        ...
        return db
                ...
                .select(
                        db.select(
                                ...
                                // An infrastructure node and a scheduled stop point use the same
                                // external identifier
                                // which is called soltunnus in Jore3 database. You can verify this
                                // by taking a look
                                // at the jr_solmu and jr_pysakki tables found from the Jore3
                                // database.
                                .join(INFRASTRUCTURE_NODES)
                                .on(
                                        INFRASTRUCTURE_NODES.INFRASTRUCTURE_NODE_EXT_ID.eq(
                                                STAGING_TABLE.SCHEDULED_STOP_POINT_EXT_ID))

                                // Use left-join because all stop points are not associated with
                                // timing places.
                                .leftJoin(NETWORK_PLACES)
                                .on(
                                        NETWORK_PLACES.NETWORK_PLACE_EXT_ID.eq(
                                                STAGING_TABLE.NETWORK_PLACE_EXT_ID))
                                .whereNotExists(
                                        db.selectOne()
                                                .from(TARGET_TABLE)
                                                .where(
                                                        TARGET_TABLE.SCHEDULED_STOP_POINT_EXT_ID.eq(
                                                                STAGING_TABLE
                        ...
                        TARGET_TABLE
                                ...
                                .and(
                                        TARGET_TABLE
                                                ...
                                                .or(
                                                        TARGET_TABLE.USAGE_IN_ROUTES.notEqual(
                                                                STAGING_TABLE.USAGE_IN_ROUTES))))
                .returningResult(TARGET_TABLE.SCHEDULED_STOP_POINT_ID)
                .fetch()
                .stream()
                    ...
                    idMappings.forEach(
                            idMapping -> {
                                c.dsl()
                                        .update(TARGET_TABLE)
                                        .set(
                                                TARGET_TABLE.SCHEDULED_STOP_POINT_JORE4_ID,
                                                idMapping.jore4Id())
                                        .where(
                                                TARGET_TABLE.SCHEDULED_STOP_POINT_EXT_ID.eq(
