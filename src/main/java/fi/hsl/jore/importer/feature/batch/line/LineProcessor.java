...
public class LineProcessor implements ItemProcessor<JrLine, PersistableLine> {
    ...
    public PersistableLine process(final JrLine item) {
        ...
        final LegacyPublicTransportDestination lineLegacyPublicTransportDestination =
                item.lineId().destination();

        return PersistableLine.of(
                ExternalIdUtil.forLine(item),
                lineNumber,
                TransitTypeToNetworkTypeMapper.resolveNetworkType(item.transitType()),
                LineClassificationUtil.resolveTypeOfLine(
                        item.transitType(), item.isTrunkLine(), item.publicTransportType(), lineNumber),
