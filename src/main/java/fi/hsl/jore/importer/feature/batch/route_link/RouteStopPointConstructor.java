...
public final class RouteStopPointConstructor {
    ...
    private static final Function<StopPointContext, StopPointContext>
            ADD_STOP_POINTS_FROM_STARTING_NODES =
                    ctx -> {
                        final Vector<Jore3RouteStopPoint> points =
                                ctx.linksAndAttributes()
                                        .routeLinks()
                                        .filter(link -> link.startNodeType() == NodeType.BUS_STOP)
                                        .zipWithIndex()
                                        .map(
                                                indexAndLink ->
                                                        fromLink(indexAndLink._1, indexAndLink._2));
                        return ctx.withStopPoints(ctx.stopPoints().appendAll(points));
                    };

    private static final Function<StopPointContext, StopPointContext>
            ADD_STOP_POINT_FROM_LAST_LINK =
                    ctx -> {
                        final LastLinkAttributes attributes =
                                ctx.linksAndAttributes().lastLinkAttributes();

                        if (attributes.nodeType() == NodeType.BUS_STOP) {
                            final int lastIndex = ctx.stopPoints().size();

                            return ctx.withStopPoints(
                                    ctx.stopPoints()
                                            .append(
                                                    fromLastLink(
                                                            ctx.linksAndAttributes()
    ...
    private static final Function<StopPointContext, StopPointContext>
            MARK_FIRST_STOP_POINT_AS_HASTUS_POINT =
                    ctx -> ctx.updateHead(sp -> sp.withHastusStopPoint(true));

    private static final Function<StopPointContext, StopPointContext>
            MARK_LAST_STOP_POINT_AS_HASTUS_POINT =
    ...
    public static Vector<Jore3RouteStopPoint> extractStopPoints(
            final RouteLinksAndAttributes entity) {
    ...
    public interface StopPointContext {
         ...
         */
        default StopPointContext updateHead(
                final Function<Jore3RouteStopPoint, Jore3RouteStopPoint> f) {
         ...
         * @return A new instance of the context
         */
        default StopPointContext updateTail(
                final Function<Jore3RouteStopPoint, Jore3RouteStopPoint> f) {
            if (stopPoints().isEmpty()) {
                return this;
            }
