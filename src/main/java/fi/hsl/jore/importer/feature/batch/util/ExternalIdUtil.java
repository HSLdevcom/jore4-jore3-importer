...
public final class ExternalIdUtil {
    ...
    public static ExternalId forLink(
            final TransitType transitType, final NodeId from, final NodeId to) {
        return ExternalId.of(
                String.format("%d-%s-%s", transitType.value(), from.value(), to.value()));
    }
    ...
    public static ExternalId forLineHeader(final LineId lineId, final LocalDate validFrom) {
        return ExternalId.of(
                String.format("%s-%s", lineId.originalValue(), validFrom.format(FORMAT)));
    }
