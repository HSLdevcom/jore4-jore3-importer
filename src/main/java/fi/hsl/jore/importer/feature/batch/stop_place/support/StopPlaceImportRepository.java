...
@Repository
public class StopPlaceImportRepository extends AbstractImportRepository<Jore3StopPlace, StopPlacePK>
        implements IStopPlaceImportRepository {

    private static final StopPlacesStaging STAGING_TABLE = StopPlacesStaging.STOP_PLACES_STAGING;
    private static final StopPlaces TARGET_TABLE = StopPlaces.STOP_PLACES;

    private static final List<String> ALLOWED_TRANSPORT_MODES =
            List.of(NetworkType.ROAD.label(), NetworkType.TRAM_TRACK.label());
    ...
    protected Set<StopPlacePK> insert() {
        return db
                ...
                .columns(
                        ...
                        TARGET_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE)
                .select(db.select(
                                ...
                                STAGING_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE)
                        .from(STAGING_TABLE)
                        .where(STAGING_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE.in(ALLOWED_TRANSPORT_MODES))
                        .andNotExists(db.selectOne()
                                .from(TARGET_TABLE)
    ...
    protected Set<StopPlacePK> update() {
        return db
                ...
                .set(TARGET_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE, STAGING_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE)
                .from(STAGING_TABLE)
                .where(STAGING_TABLE
                        .STOPS_STOP_PLACE_TRANSPORT_MODE
                        .in(ALLOWED_TRANSPORT_MODES)
                        .and(TARGET_TABLE.STOPS_STOP_PLACE_EXT_ID.eq(STAGING_TABLE.STOPS_STOP_PLACE_EXT_ID))
                        .and(TARGET_TABLE
                                .STOPS_STOP_PLACE_NAME
                                .notEqual(STAGING_TABLE.STOPS_STOP_PLACE_NAME)
                                .or(TARGET_TABLE.STOPS_STOP_PLACE_LONG_NAME.notEqual(
                                        STAGING_TABLE.STOPS_STOP_PLACE_LONG_NAME))
                                .or(TARGET_TABLE.STOPS_STOP_PLACE_LOCATION.notEqual(
                                        STAGING_TABLE.STOPS_STOP_PLACE_LOCATION))
                                .or(TARGET_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE.notEqual(
                                        STAGING_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE))))
                .returningResult(TARGET_TABLE.STOPS_STOP_PLACE_ID)
                .fetch()
                .stream()
    ...
    protected Set<StopPlacePK> delete() {
        return db
                ...
                .where(TARGET_TABLE.STOPS_STOP_PLACE_TRANSPORT_MODE.in(ALLOWED_TRANSPORT_MODES))
                .andNotExists(db.selectOne()
                        .from(STAGING_TABLE)
