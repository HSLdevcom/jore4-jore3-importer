...
@Repository
public class NodeImportRepository extends AbstractImportRepository<Jore3Node, NodePK>
    ...
    private static final InfrastructureNodes TARGET_TABLE =
        ...
        return db
                ...
                .whereNotExists(
                        selectOne()
                                .from(STAGING_TABLE)
                                .where(
                                        STAGING_TABLE.INFRASTRUCTURE_NODE_EXT_ID.eq(
    ...
    protected Set<NodePK> update() {
        return db
                ...
                // Find source rows
                .where(
                        TARGET_TABLE.INFRASTRUCTURE_NODE_EXT_ID.eq(
                                STAGING_TABLE.INFRASTRUCTURE_NODE_EXT_ID))
                ...
                .and( // Locations are different
                        (geometryEquals(
                                ...
                                // NOTE: Projected location may be null, so we can't just compare
                                // it.

                                // It might be a bit too pedantic to consider all these NULL cases,
                                // but at least this shouldn't break if we encounter NULLs in either
                                // table,
                                        ...
                                        (TARGET_TABLE.INFRASTRUCTURE_NODE_PROJECTED_LOCATION
                                                ...
                                                .and(
                                                        STAGING_TABLE
                                                                ...
                                                                .isNotNull()))
                                // or projected location is not empty in target but is empty in
                                // staging
                                .or(
                                        (TARGET_TABLE.INFRASTRUCTURE_NODE_PROJECTED_LOCATION
                                                        .isNotNull())
