...
/**
 * Contains the information of a scheduled stop point which is read from the <code>jr_pysakki</code>
 * table of the Jore3 database.
 */
@Value.Immutable
@JoreDtoStyle
...
public interface JrScheduledStopPoint extends IHasPrimaryKey<JrScheduledStopPointPK> {
    ...
    @JoreColumn(name = "elynumero", example = "1234567890")
    Optional<Long> elyNumber();

    @JoreColumn(name = "pysnimi", example = "Ritarihuone")
    Optional<String> nameFinnish();

    @JoreColumn(name = "pysnimir", nullable = true, example = "Riddarhuset")
    Optional<String> nameSwedish();

    @JoreColumn(name = "sollistunnus", example = "1234")
    Optional<String> shortId();

    @JoreColumn(name = "solkirjain", example = "H")
    Optional<String> shortLetter();

    @JoreColumn(name = "paitunnus", example = "1ELIEL")
    Optional<String> placeExternalId();

    /** Number of times used in routes */
    @JoreColumn(name = "usage_in_routes", example = "2")
    int usageInRoutes();

    static JrScheduledStopPoint of(
            final NodeId nodeId,
