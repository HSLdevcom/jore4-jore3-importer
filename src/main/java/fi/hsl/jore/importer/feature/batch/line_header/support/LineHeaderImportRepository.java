        ...
        return db
                ...
                .whereNotExists(
                        selectOne()
                                .from(STAGING_TABLE)
                                .where(
                                        STAGING_TABLE.NETWORK_LINE_HEADER_EXT_ID.eq(
    ...
    protected Set<LineHeaderPK> update() {
        return db
                ...
                .set(
                        ...
                        STAGING_TABLE.NETWORK_LINE_HEADER_ORIGIN_2)
                // Note that start date cannot update (it's part of the external id), but the end
                // date can
                .set(
                ...
                // Find source rows..
                .where(
                        TARGET_TABLE.NETWORK_LINE_HEADER_EXT_ID.eq(
                                STAGING_TABLE.NETWORK_LINE_HEADER_EXT_ID))
                // .. with updated fields
                .and(
                        (TARGET_TABLE.NETWORK_LINE_HEADER_NAME.ne(
                                        STAGING_TABLE.NETWORK_LINE_HEADER_NAME))
                                .or(
                                        TARGET_TABLE.NETWORK_LINE_HEADER_NAME_SHORT.ne(
                                                STAGING_TABLE.NETWORK_LINE_HEADER_NAME_SHORT))
                                .or(
                                        TARGET_TABLE.NETWORK_LINE_HEADER_ORIGIN_1.ne(
                                                STAGING_TABLE.NETWORK_LINE_HEADER_ORIGIN_1))
                                .or(
                                        TARGET_TABLE.NETWORK_LINE_HEADER_ORIGIN_2.ne(
                                                STAGING_TABLE.NETWORK_LINE_HEADER_ORIGIN_2))
                                .or(
                                        TARGET_TABLE.NETWORK_LINE_HEADER_VALID_DATE_RANGE.ne(
                                                STAGING_TABLE
                                                        .NETWORK_LINE_HEADER_VALID_DATE_RANGE)))
                .returningResult(TARGET_TABLE.NETWORK_LINE_HEADER_ID)
                .fetch()
                .stream()
                .map(row -> LineHeaderPK.of(row.value1()))
                    ...
                    idMappings.forEach(
                            idMapping -> {
                                c.dsl()
                                        ...
                                        .where(
                                                TARGET_TABLE.NETWORK_LINE_HEADER_EXT_ID.eq(
                                                        idMapping.externalIdOfLineHeader()))
                                        .execute();
                            });
                });
    }
