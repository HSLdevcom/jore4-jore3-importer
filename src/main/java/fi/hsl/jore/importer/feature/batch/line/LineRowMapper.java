...
public class LineRowMapper implements RowMapper<JrLine> {
    ...
    @Override
    @Nullable
    public JrLine mapRow(final ResultSet rs, final int rowNum) throws SQLException {
        return JrLine.of(
                LineId.from(getStringOrThrow(rs, "lintunnus")),
                getOptionalString(rs, "linverkko").flatMap(TransitType::of).orElse(TransitType.UNKNOWN),
