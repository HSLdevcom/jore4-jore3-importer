SELECT (
           SELECT string_agg(sq1.scheduled_stop_point_ext_id, ',' ORDER BY sq1.usage_in_routes DESC) FROM network.scheduled_stop_points sq1
           WHERE sq1.scheduled_stop_point_short_id = s.scheduled_stop_point_short_id
       ) AS external_id,
       (
           SELECT string_agg(sq1.scheduled_stop_point_ely_number::text, ',' ORDER BY sq1.usage_in_routes DESC) FROM network.scheduled_stop_points sq1
           WHERE sq1.scheduled_stop_point_short_id = s.scheduled_stop_point_short_id
       ) AS ely_number,
