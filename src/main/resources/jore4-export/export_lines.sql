...
-- that do not overlap (within one line). Originally, it was thought that a
-- separate line instance would be created for each line header into the Jore4
-- database. However, it appears that Jore3 route directions
-- (`network_route_directions` table) may overlap with multiple line headers.
-- This causes a transformation problem, because in Jore4 the validity period
-- of a route MUST NOT extend further into the past or future than the line to
-- which the route belongs.
...
--     that no (transformed) route overlaps multiple line instances transformed
--     to Jore4.
...
-- as an associated route direction, causing database constraint violations
-- during the route export stage if no date corrections are made. Therefore,
-- for each line, the latest validity date of the route directions associated
-- with it is determined, and if necessary, it is used as the end date of the
-- last header of the line, if it is later than what is originally in the
-- header.
    ...
    FROM (
        SELECT
            *,
            ...
            -- This is the cumulative sum over an expanding set towards the end. That's why ordering
            -- is important here.
...
line_header_clusters_with_end_date_mod AS (
    ...
    -- directions can fit within it. This only needs to be done for the last line header cluster of
