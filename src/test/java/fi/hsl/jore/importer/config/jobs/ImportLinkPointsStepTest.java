...
public class ImportLinkPointsStepTest extends BatchIntegrationTest {
    ...
    @Autowired private ILinkShapeTestRepository shapeRepository;

    @Test
    public void
            givenSampleDataInSourceDatabase_andAnEmptyDatabase_whenImportLinkPointsStepsAreRun_thenLinkShapeForLinkIsImported() {
        assertThat(shapeRepository.empty(), is(true));

        runSteps(STEPS);

        assertThat(shapeRepository.count(), is(1));

        final LinkShape shape =
                shapeRepository.findByExternalId(ExternalId.of("1-c-d")).orElseThrow();
        ...
        final LineString expectedPoints =
                GeometryUtil.toLineString(
                        GeometryUtil.SRID_WGS84, List.of(nodeC, poCD1, poCD2, nodeD));

        assertThat(geometriesMatch(shape.geometry(), expectedPoints), is(true));
    }
}
