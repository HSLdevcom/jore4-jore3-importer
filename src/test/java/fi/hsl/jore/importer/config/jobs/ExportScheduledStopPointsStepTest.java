...
class ExportScheduledStopPointsStepTest extends BatchIntegrationTest {
    ...
    private static final String EXPECTED_INFRASTRUCTURE_LINK_ID =
    ...
    private static final String EXPECTED_SCHEDULED_STOP_POINT_VEHICLE_MODE =
            VehicleMode.BUS.getValue();

    private static final List<String> STEPS = List.of("exportScheduledStopPointsStep");

    private static final fi.hsl.jore.importer.jooq.network.tables.ScheduledStopPoints
            IMPORTER_SCHEDULED_STOP_POINT =
                    fi.hsl.jore.importer.jooq.network.Tables.SCHEDULED_STOP_POINTS;
    private static final fi.hsl.jore.jore4.jooq.service_pattern.tables.ScheduledStopPoint
            JORE4_SCHEDULED_STOP_POINT =
    ...
    @Autowired
    ExportScheduledStopPointsStepTest(
        ...
        this.jore4ScheduledStopPointTargetTable =
                new Table(jore4DataSource, "service_pattern.scheduled_stop_point");
        ...
        this.testRepository =
                new Jore4ValidityPeriodTestRepository(
                        jore4DataSource, ValidityPeriodTargetTable.SCHEDULED_STOP_POINT);
    }
    ...
    @Test
    @DisplayName(
            "Should save the exported scheduled stop point with the correct infrastructure link id")
    ...
    void shouldSaveNewScheduledStopPointWithCorrectMeasuredLocation(SoftAssertions softAssertions) {
        ...
        final ScheduledStopPointTestLocation measuredLocation =
                jdbcTemplate
                        .query(
                                ScheduledStopPointTestLocation.SQL_QUERY_GET_MEASURED_LOCATION,
                                (resultSet, i) ->
                                        new ScheduledStopPointTestLocation(
                                                resultSet.getDouble(
                                                        ScheduledStopPointTestLocation
                                                                .SQL_ALIAS_X_COORDINATE),
                                                resultSet.getDouble(
                                                        ScheduledStopPointTestLocation
                                                                .SQL_ALIAS_Y_COORDINATE)))
                        .get(0);

        softAssertions.assertThat(measuredLocation.getX()).as("x").isEqualTo(X_COORDINATE);
        softAssertions.assertThat(measuredLocation.getY()).as("y").isEqualTo(Y_COORDINATE);
    }
    ...
    @Test
    @DisplayName(
            "Should save the exported scheduled stop point with the correct validity period start"
                    + " time")
    ...
    @Test
    @DisplayName(
            "Should save the exported scheduled stop point with the correct validity period end"
                    + " time")
    ...
    @Test
    @DisplayName(
            "Should update the Jore 4 id of the scheduled stop point found from the importer's"
                    + " database")
    ...
    @Test
    @DisplayName(
            "Should save the exported scheduled stop point vehicle mode with the generated"
                    + " scheduled stop point id")
