...
public class ImportRoutesDirectionsStepTest extends BatchIntegrationTest {
    ...
    public void whenImportingLinesToEmptyDb_thenInsertsExpectedLine() {
        assertThat(routeDirectionRepository.empty(), is(true));

        runSteps(STEPS);

        assertThat(routeDirectionRepository.count(), is(1));

        final RouteDirection routeDirection = routeDirectionRepository.findAll().get(0);

        assertThat(routeDirection.alive(), is(true));
        assertThat(routeDirection.externalId(), is(ExternalId.of("1001-1-20200603")));
        assertThat(routeDirection.direction(), is(DirectionType.OUTBOUND));
        assertThat(
                routeDirection.name(),
        ...
        assertThat(
                routeDirection.nameShort(),
                is(MultilingualString.empty()
                        .with(FINNISH, "Ktori-EteläHervanta")
                        .with(SWEDISH, "CentralT-S.Hervanta")));
        assertThat(
                routeDirection.origin(),
                is(MultilingualString.empty().with(FINNISH, "Keskustori").with(SWEDISH, "Central torget")));
        assertThat(
                routeDirection.destination(),
                is(MultilingualString.empty().with(FINNISH, "Etelä-Hervanta").with(SWEDISH, "Södra Hervanta")));
        assertThat(routeDirection.lengthMeters().orElseThrow(), is(100));
        assertThat(
                routeDirection.validTime(), is(DateRange.between(LocalDate.of(2020, 6, 3), LocalDate.of(2021, 7, 5))));
    }
}
