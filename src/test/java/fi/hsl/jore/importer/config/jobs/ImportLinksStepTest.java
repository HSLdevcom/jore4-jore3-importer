...
public class ImportLinksStepTest extends BatchIntegrationTest {
    ...
    // Note that links always point to the actual location, not the projected location (when dealing
    // with bus stops)
    ...
    private static final List<Tuple3<ExternalId, LineString, NetworkType>> LINKS =
            List.of(
                    Tuple.of(
                            ExternalId.of("1-c-d"),
                            GeometryUtil.toLineString(
                                    GeometryUtil.SRID_WGS84, List.of(NODE_C, NODE_D)),
                            NetworkType.ROAD),
                    Tuple.of(
                            ExternalId.of("1-d-e"),
                            GeometryUtil.toLineString(
                                    GeometryUtil.SRID_WGS84, List.of(NODE_D, NODE_E)),
                            NetworkType.ROAD),
                    Tuple.of(
                            ExternalId.of("1-e-f"),
                            GeometryUtil.toLineString(
                                    GeometryUtil.SRID_WGS84, List.of(NODE_E, NODE_F)),
                            NetworkType.ROAD));
    ...
    @Test
    public void
            givenSampleDataInSourceDatabase_andAnEmptyDatabase_whenImportLinksStepsAreRun_thenLinksBetweenNodesCAndDAndNodesDAndEAndNodesEAndFAreImported() {
