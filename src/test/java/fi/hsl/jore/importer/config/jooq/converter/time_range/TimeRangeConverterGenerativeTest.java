...
public class TimeRangeConverterGenerativeTest {
    ...
    public static final class Generators {
        ...
        public static Gen<Instant> timestamp(final Instant start,
        ...
        public static Gen<Optional<Instant>> maybeTimestamp(final Instant start,
                                                            final Duration range) {
        ...
        public static Gen<TimeRange> randomTimeRange() {
            return Generate.oneOf(emptyTimeRange(),
                                  unboundedTimeRange(),
                                  instantTimeRange());
        }
    }
    ...
    public void testTimeRangeRoundTrip() {
        qt().withExamples(1000)
            .forAll(Generators.randomTimeRange())
            .checkAssert(timeRange -> {
                final Object converted = CONVERTER.to(timeRange);
                final TimeRange roundTrip = CONVERTER.from(converted);

                assertThat(String.format("'%s' should match '%s'", timeRange, roundTrip),
                           timeRange,
                           is(roundTrip));
            });
    }
