...
@Sql(
        ...
        config =
                @SqlConfig(
                        ...
                        transactionManager = "sourceTransactionManager"))
...
public class ImportRouteLinksStepTest extends BatchIntegrationTest {
    ...
    private static final List<Tuple4<ExternalId, Integer, ExternalId, ExternalId>> ROUTE_POINTS =
            List.of(
                    Tuple.of(
                    ...
                    Tuple.of(
                            ExternalId.of("1338-d"),
                            1,
                            ExternalId.of("d"),
                            ExternalId.of("1001-1-20200603")),
                    Tuple.of(
                            ExternalId.of("1339-e"),
                            2,
                            ExternalId.of("e"),
                            ExternalId.of("1001-1-20200603")),
    ...
    // and the external id of the route direction
    private static final List<Tuple4<ExternalId, Integer, ExternalId, ExternalId>> ROUTE_LINKS =
            List.of(
                    Tuple.of(
                            ExternalId.of("1337"),
                            0,
    ...
    @Autowired private IRoutePointTestRepository routePointRepository;

    @Autowired private IRouteStopPointTestRepository routeStopPointTestRepository;

    @Autowired private IRouteLinkTestRepository routeLinkImportRepository;

    @Autowired private INodeTestRepository nodeRepository;

    @Autowired private ILinkTestRepository linkRepository;

    @Autowired private IRouteDirectionTestRepository routeDirectionRepository;
    ...
    private void assertRoutePoints() {
        ROUTE_POINTS.forEach(
                expectedRoutePointParams -> {
                    ...
                    assertThat(
                            String.format(
                                    "route point %s should have correct order number", externalId),
                    ...
                    assertThat(
                            String.format(
                                    "route point %s should refer to node %s",
                                    externalId, nodeExternalId),
                            node.externalId(),
                            is(nodeExternalId));

                    final RouteDirection routeDirection =
                            routeDirectionRepository
                    ...
                    assertThat(
                            String.format(
                                    "route point %s should refer to route direction %s",
    ...
    private void assertRouteStopPoints() {
        ROUTE_STOP_POINTS.forEach(
                expectedStopPointParams -> {
                    ...
                    assertThat(
                            String.format(
                                    "stop point %s should have correct order number", externalId),
                            stopPoint.orderNumber(),
                            is(expectedStopPointParams._2));

                    assertThat(
                            String.format(
                                    "stop point %s should have correct Hastus point flag",
                                    externalId),
                    ...
                    assertThat(
                            String.format(
                                    "stop point %s should have correct regulated timing point"
                                            + " status",
                                    externalId),
                    ...
                    assertThat(
                            String.format(
                                    "stop point %s should have correct via point flag", externalId),
                            stopPoint.viaPoint(),
                    ...
                    if (expectedStopPointParams._6 != null) {
                        ...
                        assertThat(
                                String.format(
                                        "stop point %s should have correct Finnish via name",
                                        externalId),
                        ...
                        assertThat(
                                String.format(
                                        "stop point %s should have correct Swedish via name",
                                        externalId),
                    ...
                    assertThat(
                            String.format(
                                    "stop point %s should have correct timetable column",
                                    externalId),
    ...
    private void assertRouteLinks() {
        ROUTE_LINKS.forEach(
                expectedLinksParams -> {
                    ...
                    assertThat(
                            String.format(
                                    "route link %s should have correct order number", externalId),
                    ...
                    assertThat(
                            String.format(
                                    "route link %s should refer to link %s",
                    ...
                    assertThat(
                            String.format(
                                    "route link %s should refer to route direction %s",
                                    externalId, routeDirectionExternalId),
                            routeDirection.externalId(),
                            is(routeDirectionExternalId));
                });
