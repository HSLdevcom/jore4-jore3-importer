...
public class ImportNodesStepTest extends BatchIntegrationTest {
    ...
    private static final List<String> STEPS =
            List.of("prepareNodesStep", "importNodesStep", "commitNodesStep");

    // External ID of the node,
    // The node location
    // The node projected location,
    // and the node type
    private static final List<Tuple4<ExternalId, Point, Optional<Point>, NodeType>> NODES =
            List.of(
                    Tuple.of(
                            ExternalId.of("c"),
                            ...
                            Optional.of(
                                    GeometryUtil.toPoint(
                                            GeometryUtil.SRID_WGS84, new Coordinate(13, 12))),
                            NodeType.STOP),
                    Tuple.of(
                            ExternalId.of("d"),
                    ...
                    Tuple.of(
                            ...
                            Optional.of(
                                    GeometryUtil.toPoint(
                                            GeometryUtil.SRID_WGS84, new Coordinate(21, 20))),
                            NodeType.STOP));
    ...
    public void
        ...
        NODES.forEach(
                expectedNodeParams -> {
                    ...
                    assertThat(
                            String.format(
                                    "node %s should have correct projected location", externalId),
