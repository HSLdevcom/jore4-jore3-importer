...
public class TimeRangeConverterTest {
    ...
    public void givenBounded_thenReturnBounded() {
        assertThat(
                CONVERTER.from(String.format("[%s,%s]", PG_STRING_START, PG_STRING_END)),
                is(TimeRange.of(Range.closed(TIMESTAMP_START, TIMESTAMP_END))));
        assertThat(
                CONVERTER.from(String.format("(%s,%s)", PG_STRING_START, PG_STRING_END)),
                is(TimeRange.of(Range.open(TIMESTAMP_START, TIMESTAMP_END))));
    }

    @Test
    public void givenOnlyTo_thenReturnBounded() {
        assertThat(
                CONVERTER.from(String.format("[,%s]", PG_STRING_END)), is(TimeRange.of(Range.atMost(TIMESTAMP_END))));
        assertThat(
                CONVERTER.from(String.format("(,%s)", PG_STRING_END)), is(TimeRange.of(Range.lessThan(TIMESTAMP_END))));
        assertThat(
                CONVERTER.from(String.format("[,%s)", PG_STRING_END)), is(TimeRange.of(Range.lessThan(TIMESTAMP_END))));
        assertThat(
                CONVERTER.from(String.format("(,%s]", PG_STRING_END)), is(TimeRange.of(Range.atMost(TIMESTAMP_END))));
    ...
    public void givenOnlyFrom_thenReturnBounded() {
        assertThat(
                CONVERTER.from(String.format("[%s,]", PG_STRING_START)),
    ...
    public void givenStringWithQuotes_thenReturnParsed() {
        assertThat(
                ...
                is(TimeRange.of(Range.atLeast(TIMESTAMP_START))));
        assertThat(
                "unbalanced starting quote is ignored",
                CONVERTER.from(String.format("\"[%s,)", PG_STRING_START)),
                is(TimeRange.of(Range.atLeast(TIMESTAMP_START))));
        assertThat(
                "unbalanced ending quote is ignored",
                CONVERTER.from(String.format("[%s,)\"", PG_STRING_START)),
                is(TimeRange.of(Range.atLeast(TIMESTAMP_START))));
    }
    ...
    public void givenMissingTimezone_thenThrows() {
        assertThrows(DateTimeParseException.class, () -> CONVERTER.from(String.format("[%s,)", "2004-10-19 10:23:54")));
    }

    @Test
    public void givenMissingTime_thenThrows() {
        assertThrows(DateTimeParseException.class, () -> CONVERTER.from(String.format("[%s,)", "2004-10-19")));
    }

    @Test
    public void givenMissingDate_thenThrows() {
        assertThrows(DateTimeParseException.class, () -> CONVERTER.from(String.format("[%s,)", "10:23:54+02")));
    }
}
