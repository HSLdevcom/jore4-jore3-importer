...
@Sql(
        scripts = {"/sql/jore3/drop_tables.sql", "/sql/jore3/populate_lines.sql"},
        config =
                @SqlConfig(
...
public class ImportLinesStepTest extends BatchIntegrationTest {
    ...
    private static final List<String> STEPS =
            List.of("prepareLinesStep", "importLinesStep", "commitLinesStep");

    @Autowired private ILineTestRepository lineRepository;

    @Test
    public void whenImportingLinesToEmptyDb_thenInsertsExpectedLine() {
        assertThat(lineRepository.empty(), is(true));
        ...
        assertThat(line.externalId(), is(ExternalId.of("1001")));
        assertThat(line.lineNumber(), is("1"));
        assertThat(line.networkType(), is(NetworkType.ROAD));
        assertThat(line.typeOfLine(), is(TypeOfLine.EXPRESS_BUS_SERVICE));
    }
}
