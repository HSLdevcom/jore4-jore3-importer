...
class ExportLineStepTest extends BatchIntegrationTest {
    ...
    private static final String EXPECTED_NAME =
            "{\"fi_FI\":\"Eira - Töölö - Sörnäinen (M) - Käpylä\",\"sv_SE\":\"Eira - Tölö - Sörnäs"
                    + " (M) - Kottby\"}";
    ...
    private static final fi.hsl.jore.importer.jooq.network.tables.NetworkLineHeaders
            IMPORTER_LINE_HEADER = Tables.NETWORK_LINE_HEADERS;
    private static final fi.hsl.jore.jore4.jooq.route.tables.Line JORE4_LINE =
            fi.hsl.jore.jore4.jooq.route.Tables.LINE;
    ...
    ExportLineStepTest(
        ...
        this.importerLineHeaderTable =
                new Table(importerDataSource, "network.network_line_headers");
        this.jore4LineTable = new Table(jore4DataSource, "route.line");
        this.testRepository =
                new Jore4ValidityPeriodTestRepository(
                        jore4DataSource, ValidityPeriodTargetTable.LINE);
    }
    ...
    @Test
    @DisplayName(
            "Should update the Jore 4 ID to the line header found from the importer's database")
