    ...
    class CreateMapMatchingRequest {
        ...
        void createRouteGeometry() {
            ...
            final LineString routeLineString =
                    new LineString(coordinates, GeometryUtil.factoryForSrid(GeometryUtil.SRID_WGS84));

            routeGeometryInput = ImporterRouteGeometry.from(
                    routeLineString, ROUTE_DIRECTION_ID, ROUTE_DIRECTION_EXT_ID, ROUTE_JORE4_ID);
        }
        ...
        class WhenInputRoutePointIsCrossroad {
            ...
            void createRoutePoint() {
                final CoordinateSequence locationCoordinates = new CoordinateArraySequence(
                        new Coordinate[] {new Coordinate(ROUTE_POINT_LNG, ROUTE_POINT_LAT)});
                final Point location =
                        new Point(locationCoordinates, GeometryUtil.factoryForSrid(GeometryUtil.SRID_WGS84));

                routePoint = ImporterRoutePoint.from(
                        location, 1, Optional.empty(), NodeType.CROSSROADS, Optional.empty(), Optional.empty());
            }
            ...
            void shouldReturnMapMatchingRequestThatHasOneRoutePoint() {
                final MapMatchingRequestDTO request =
                        MapMatchingRequestBuilder.createMapMatchingRequest(routeGeometryInput, List.of(routePoint));
            ...
            void shouldReturnMapMatchingRequestWithCorrectRoutePoint(SoftAssertions softAssertions) {
                ...
                softAssertions.assertThat(routePoint.getType()).as("type").isEqualTo(RoutePointType.ROAD_JUNCTION);
        ...
        class WhenInputRoutePointIsScheduledStopPoint {
            ...
            void createLocations() {
                final CoordinateSequence locationCoordinates = new CoordinateArraySequence(
                        new Coordinate[] {new Coordinate(ROUTE_POINT_LNG, ROUTE_POINT_LAT)});
                location = new Point(locationCoordinates, GeometryUtil.factoryForSrid(GeometryUtil.SRID_WGS84));

                final CoordinateSequence projectedLocationCoordinates = new CoordinateArraySequence(
                        new Coordinate[] {new Coordinate(ROUTE_PROJECTED_POINT_LNG, ROUTE_PROJECTED_POINT_LAT)});
                projectedLocation =
                        new Point(projectedLocationCoordinates, GeometryUtil.factoryForSrid(GeometryUtil.SRID_WGS84));
            }
            ...
            class WhenElyNumberIsInteger {
                ...
                void shouldReturnMapMatchingRequestWithCorrectRoutePoint(final SoftAssertions softAssertions) {
                    final MapMatchingRequestDTO request =
                            MapMatchingRequestBuilder.createMapMatchingRequest(routeGeometryInput, List.of(routePoint));
                    final RoutePointRequestDTO routePoint =
                            request.getRoutePoints().get(0);
