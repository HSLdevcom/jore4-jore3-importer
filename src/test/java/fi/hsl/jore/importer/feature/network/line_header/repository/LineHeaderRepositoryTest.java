...
public class LineHeaderRepositoryTest extends IntegrationTest {
    ...
    public LineHeaderRepositoryTest(
    ...
    public void beforeEach() {
        assertThat(
                ...
                is(true));
        assertThat(
                "Line header repository should be empty at the start of the test",
    ...
    public void whenPersistingLine_thenPersistedLineIsEqual() {
        ...
        final LineHeaderPK id =
                lineHeaderRepository.insert(
        ...
        assertThat(lineHeaderRepository.count(), is(1));

        final Optional<LineHeader> maybeLineHeader = lineHeaderRepository.findById(id);

        assertThat(maybeLineHeader.isPresent(), is(true));

        final LineHeader lineHeader = maybeLineHeader.orElseThrow();

        assertThat(lineHeader.alive(), is(true));
        assertThat(lineHeader.externalId(), is(HEADER_EXT_ID));
        assertThat(lineHeader.name(), is(NAME));
        assertThat(lineHeader.nameShort(), is(NAME_SHORT));
        assertThat(lineHeader.origin1(), is(ORIGIN_1));
        assertThat(lineHeader.origin2(), is(ORIGIN_2));
        assertThat(lineHeader.validTime(), is(VALID_TIME));
    }
}
