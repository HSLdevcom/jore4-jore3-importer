...
public class RouteGeometryTestFactory {
    ...
    public static ImporterRouteGeometry createRouteGeometry() {
        ...
        return ImporterRouteGeometry.from(routeLineString, ROUTE_DIRECTION_ID, ROUTE_DIRECTION_EXT_ID, ROUTE_JORE4_ID);
    }

    public static List<ImporterRoutePoint> createRoutePoints() {

        return List.of(createStopPoint(), createJunctionPoint());
    }
    ...
    private static ImporterRoutePoint createStopPoint() {
        final CoordinateSequence locationCoordinates = new CoordinateArraySequence(
                new Coordinate[] {new Coordinate(REQUEST_ROUTE_STOP_POINT_POINT_X, REQUEST_ROUTE_STOP_POINT_POINT_Y)});
        final Point location = new Point(locationCoordinates, GeometryUtil.factoryForSrid(GeometryUtil.SRID_WGS84));
    ...
    private static ImporterRoutePoint createJunctionPoint() {
        final CoordinateSequence locationCoordinates = new CoordinateArraySequence(
                new Coordinate[] {new Coordinate(REQUEST_ROUTE_JUNCTION_POINT_X, REQUEST_ROUTE_JUNCTION_POINT_Y)});
        final Point location = new Point(locationCoordinates, GeometryUtil.factoryForSrid(GeometryUtil.SRID_WGS84));

        return ImporterRoutePoint.from(
                location, 2, Optional.empty(), NodeType.CROSSROADS, Optional.empty(), Optional.empty());
    }
}
