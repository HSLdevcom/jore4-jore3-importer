...
class JourneyPatternStopExportReaderTest {
    ...
    class WhenSourceTableHasOneRouteWithThreeStops {
        ...
        private final UUID JOURNEY_PATTERN_JORE4_ID =
                UUID.fromString("ec564137-f30c-4689-9322-4ef650768af3");
        ...
        private final RegulatedTimingPointStatus
                FIRST_JOURNEY_PATTERN_STOP_REGULATED_TIMING_POINT_STATUS =
        ...
        private final RegulatedTimingPointStatus
                SECOND_JOURNEY_PATTERN_STOP_REGULATED_TIMING_POINT_STATUS =
        ...
        private static final int THIRD_JOURNEY_PATTERN_STOP_ORDER_NUMBER = 3;
        ...
        private final RegulatedTimingPointStatus
                THIRD_JOURNEY_PATTERN_STOP_REGULATED_TIMING_POINT_STATUS =
                        RegulatedTimingPointStatus.YES_LOAD_TIME;
        private static final boolean THIRD_JOURNEY_PATTERN_STOP_IS_VIA_POINT = false;

        @Test
        @DisplayName(
                "The first invocation of the read() method must return the information of the"
                        + " second stop")
        void firstInvocationOfReadMethodMustReturnInformationOfFirstStop(
            ...
            softAssertions.assertThat(first.viaPointNames().isEmpty()).as("viaPointNames").isTrue();
        }

        @Test
        @DisplayName(
                "The second invocation of the read() method must return the information of the"
                        + " second stop")
        void secondInvocationOfReadMethodMustReturnInformationOfSecondStop(
                SoftAssertions softAssertions) throws Exception {
            ...
            softAssertions.assertThat(second.timingPlaceLabel()).as("timingPlaceLabel").isEmpty();
            softAssertions
            ...
            softAssertions
                    .assertThat(
                            JoreLocaleUtil.getI18nString(
                                    second.viaPointNames().get(), JoreLocaleUtil.FINNISH))
                    .as("viaPointFinnishName")
                    .isEqualTo(SECOND_JOURNEY_PATTERN_STOP_VIA_NAME_FINNISH);
            softAssertions
                    .assertThat(
                            JoreLocaleUtil.getI18nString(
                                    second.viaPointNames().get(), JoreLocaleUtil.SWEDISH))
        ...
        @Test
        @DisplayName(
                "The third invocation of the read() method must return the information of the third"
                        + " stop")
        void thirdInvocationOfReadMethodMustReturnInformationOfFirstStop(
                final SoftAssertions softAssertions) throws Exception {
            // The first invocation returns the journey pattern stop found from the database.
            final ImporterJourneyPatternStop first = reader.read();
            assertThat(first).isNotNull();
        ...
        @Test
        @DisplayName("The fourth invocation of the read method must return null")
        void fourthInvocationOfReaderMethodMustReturnNull() throws Exception {
            // The first invocation returns the journey pattern stop found from the database.
            final ImporterJourneyPatternStop first = reader.read();
            assertThat(first).isNotNull();
            ...
            // The third invocation returns the journey pattern stop found from the database.
            final ImporterJourneyPatternStop third = reader.read();
            assertThat(third).isNotNull();

            // Because there are no more journey pattern stops, this invocation must return null.
            final ImporterJourneyPatternStop fourth = reader.read();
