...
class ScheduledStopPointExportProcessorTest {
    ...
    class WhenSourceStopHasOneElyNumber {
        ...
        class WhenNoDigiroadStopIsFoundWithElyNumberOfSourceStop {
            ...
            private final ImporterScheduledStopPoint jore3Stop =
                    ImporterScheduledStopPoint.of(
                            List.of(ExternalId.of(JORE_3_STOP_EXTERNAL_ID)),
                            List.of(UNKNOWN_ELY_NUMBER),
                            JoreGeometryUtil.fromDbCoordinates(
                                    JORE_3_STOP_Y_COORDINATE, JORE_3_STOP_X_COORDINATE),
                            JoreLocaleUtil.createMultilingualString(
                                    JORE_3_STOP_FINNISH_NAME, JORE_3_STOP_SWEDISH_NAME),
        ...
        class WhenDigiroadStopIsFoundWithElyNumberOfSourceStop {
            ...
            void shouldReturnScheduledStopPointWithCorrectExternalStopId() throws Exception {
                final Jore4ScheduledStopPoint output = processor.process(jore3Stop);
                assertThat(output.externalScheduledStopPointId())
                        .isEqualTo(JORE_3_STOP_EXTERNAL_ID);
            }

            @Test
            @DisplayName(
                    "Should return a scheduled stop point with the correct external id of"
                            + " infrastructure link")
            void shouldReturnScheduledStopPointWithCorrectExternalInfrastructureLinkId()
                    throws Exception {
                final Jore4ScheduledStopPoint output = processor.process(jore3Stop);
                assertThat(output.externalInfrastructureLinkId())
                        .isEqualTo(DIGIROAD_STOP_INFRA_LINK_ID);
            }
            ...
            void shouldReturnScheduledStopPointWithCorrectStopDirection() throws Exception {
                final Jore4ScheduledStopPoint output = processor.process(jore3Stop);
                assertThat(output.directionOnInfraLink())
                        .isEqualTo(JORE4_STOP_POINT_DIRECTION_ON_INFRA_LINK);
            }
            ...
            @Test
            @DisplayName(
                    "Should return a scheduled stop point with the correct validity period start"
                            + " time")
            void shouldReturnScheduledStopPointWithCorrectValidityPeriodStartTime()
                    throws Exception {
                final Jore4ScheduledStopPoint output = processor.process(jore3Stop);
            ...
            @Test
            @DisplayName(
                    "Should return a scheduled stop point with the correct validity period end"
                            + " time")
    ...
    class WhenSourceStopHasTwoElyNumbers {
        ...
        class WhenDigiroadStopIsFoundWithSecondElyNumberOfSourceStop {
            ...
            private final ImporterScheduledStopPoint jore3Stop =
                    ImporterScheduledStopPoint.of(
                            List.of(
                                    ExternalId.of(UNKNOWN_EXTERNAL_ID),
