...
class RouteDirectionImportRepositoryTest {
    ...
    @Autowired
    RouteDirectionImportRepositoryTest(
            final IRouteDirectionImportRepository importRepository,
    ...
    class CommitStagingToTarget {
        ...
        private final UUID EXPECTED_NETWORK_ROUTE_ID =
                UUID.fromString("484d89ae-f365-4c9b-bb1a-8f7b783e95f3");
        ...
        // The expected date range is: [2021-01-01..2022-01-01). We need pass a weird end date to
        // the between() method
        // because its implementation moves the end day forward by one day.
        private final DateRange EXPECTED_VALID_DATE_RANGE =
                DateRange.between(LocalDate.parse("2021-01-01"), LocalDate.parse("2021-12-31"));
        ...
        class WhenStagingTableHasNoRows {
            ...
            class WhenTargetTableHasOneRow {
                ...
                void shouldDeleteExistingRowFromTargetTable(SoftAssertions softAssertions) {
                    final Map<RowStatus, Set<RouteDirectionPK>> result =
        ...
        class WhenStagingTableHasRows {
            ...
            class WhenTargetTableIsEmpty {
                ...
                void shouldInsertNewRouteDirectionToTargetTable(SoftAssertions softAssertions) {
                    ...
                    softAssertions
                            .assertThat(inserted.routeId().value())
                            .overridingErrorMessage(
                                    "Expected the network route id to be: %s but was: %s",
                                    EXPECTED_NETWORK_ROUTE_ID, inserted.routeId().value())
                            .isEqualTo(EXPECTED_NETWORK_ROUTE_ID);

                    softAssertions
                            .assertThat(inserted.direction())
                            .overridingErrorMessage(
                                    "Expected the direction type to be: %s but was: %s",
                                    EXPECTED_NETWORK_ROUTE_DIRECTION_TYPE, inserted.direction())
                            .isEqualTo(EXPECTED_NETWORK_ROUTE_DIRECTION_TYPE);

                    softAssertions
                            .assertThat(inserted.lengthMeters().get())
                            .overridingErrorMessage(
                                    "Expected the length of the route direction to be: %s but was:"
                                            + " %s",
                    ...
                    softAssertions
                            ...
                            .overridingErrorMessage(
                                    "Expected the route direction external id to be: %s but was:"
                                            + " %s",
                                    EXPECTED_NETWORK_ROUTE_DIRECTION_EXT_ID,
                                    inserted.externalId().value())
                            .isEqualTo(EXPECTED_NETWORK_ROUTE_DIRECTION_EXT_ID);

                    final String actualFinnishName =
                            JoreLocaleUtil.getI18nString(inserted.name(), JoreLocaleUtil.FINNISH);
                    softAssertions
                            .assertThat(actualFinnishName)
                            .overridingErrorMessage(
                                    "Expected the Finnish name to be: %s but was: %s",
                                    EXPECTED_FINNISH_NAME, actualFinnishName)
                            .isEqualTo(EXPECTED_FINNISH_NAME);

                    final String actualSwedishName =
                            JoreLocaleUtil.getI18nString(inserted.name(), JoreLocaleUtil.SWEDISH);
                    softAssertions
                            .assertThat(actualSwedishName)
                            .overridingErrorMessage(
                                    "Expected the Swedish name to be: %s but was: %s",
                                    EXPECTED_SWEDISH_NAME, actualSwedishName)
                            .isEqualTo(EXPECTED_SWEDISH_NAME);

                    final String actualFinnishNameShort =
                            JoreLocaleUtil.getI18nString(
                                    inserted.nameShort(), JoreLocaleUtil.FINNISH);
                    softAssertions
                            .assertThat(actualFinnishNameShort)
                            .overridingErrorMessage(
                                    "Expected the Finnish short name to be: %s but was: %s",
                                    EXPECTED_FINNISH_NAME_SHORT, actualFinnishNameShort)
                            .isEqualTo(EXPECTED_FINNISH_NAME_SHORT);

                    final String actualSwedishNameShort =
                            JoreLocaleUtil.getI18nString(
                                    inserted.nameShort(), JoreLocaleUtil.SWEDISH);
                    softAssertions
                            .assertThat(actualSwedishNameShort)
                            .overridingErrorMessage(
                                    "Expected the Swedish short name to be: %s but was: %s",
                                    EXPECTED_SWEDISH_NAME_SHORT, actualSwedishNameShort)
                            .isEqualTo(EXPECTED_SWEDISH_NAME_SHORT);

                    final String actualFinnishOrigin =
                            JoreLocaleUtil.getI18nString(inserted.origin(), JoreLocaleUtil.FINNISH);
                    softAssertions
                            .assertThat(actualFinnishOrigin)
                            .overridingErrorMessage(
                                    "Expected the Finnish origin to be: %s but was: %s",
                                    EXPECTED_FINNISH_ORIGIN, actualFinnishOrigin)
                            .isEqualTo(EXPECTED_FINNISH_ORIGIN);

                    final String actualSwedishOrigin =
                            JoreLocaleUtil.getI18nString(inserted.origin(), JoreLocaleUtil.SWEDISH);
                    softAssertions
                            .assertThat(actualSwedishOrigin)
                            .overridingErrorMessage(
                                    "Expected the Swedish origin to be: %s but was: %s",
                                    EXPECTED_SWEDISH_ORIGIN, actualSwedishOrigin)
                            .isEqualTo(EXPECTED_SWEDISH_ORIGIN);

                    final String actualFinnishDestination =
                            JoreLocaleUtil.getI18nString(
                                    inserted.destination(), JoreLocaleUtil.FINNISH);
                    softAssertions
                            .assertThat(actualFinnishDestination)
                            .overridingErrorMessage(
                                    "Expected the Finnish destination to be: %s but was: %s",
                                    EXPECTED_FINNISH_DESTINATION, actualFinnishDestination)
                            .isEqualTo(EXPECTED_FINNISH_DESTINATION);

                    final String actualSwedishDestination =
                            JoreLocaleUtil.getI18nString(
                                    inserted.destination(), JoreLocaleUtil.SWEDISH);
                    softAssertions
                            .assertThat(actualSwedishDestination)
                            .overridingErrorMessage(
                                    "Expected the Swedish destination to be: %s but was: %s",
                                    EXPECTED_SWEDISH_DESTINATION, actualSwedishDestination)
                            .isEqualTo(EXPECTED_SWEDISH_DESTINATION);

                    softAssertions
                            .assertThat(inserted.validTime())
                            .overridingErrorMessage(
                                    "Expected the valid data range to be: %s but was: %s",
                                    EXPECTED_VALID_DATE_RANGE, inserted.validTime())
            ...
            class WhenTargetTableContainsImportedRouteDirection {
                ...
                @Test
                @DisplayName(
                        "Should update the information of the route direction found from the target"
                                + " table")
                void shouldUpdateInformationOfRouteDirectionFoundFromTargetTable(
                        SoftAssertions softAssertions) {
                    importRepository.commitStagingToTarget();
                    final RouteDirection updated =
                            targetRepository
                                    .findById(
                                            RouteDirectionPK.of(
                    ...
                    softAssertions
                            .assertThat(updated.routeId().value())
                            .overridingErrorMessage(
                                    "Expected the network route id to be: %s but was: %s",
                                    EXPECTED_NETWORK_ROUTE_ID, updated.routeId().value())
                            .isEqualTo(EXPECTED_NETWORK_ROUTE_ID);

                    softAssertions
                            .assertThat(updated.direction())
                            .overridingErrorMessage(
                                    "Expected the direction type to be: %s but was: %s",
                                    EXPECTED_NETWORK_ROUTE_DIRECTION_TYPE, updated.direction())
                            .isEqualTo(EXPECTED_NETWORK_ROUTE_DIRECTION_TYPE);

                    softAssertions
                            .assertThat(updated.lengthMeters().get())
                            .overridingErrorMessage(
                                    "Expected the length of the route direction to be: %s but was:"
                                            + " %s",
                                    EXPECTED_NETWORK_ROUTE_DIRECTION_LENGTH,
                    ...
                    final String actualFinnishNameShort =
                            JoreLocaleUtil.getI18nString(
                                    updated.nameShort(), JoreLocaleUtil.FINNISH);
                    softAssertions
                            .assertThat(actualFinnishNameShort)
                            .overridingErrorMessage(
                                    "Expected the Finnish short name to be: %s but was: %s",
                    ...
                    final String actualSwedishNameShort =
                            JoreLocaleUtil.getI18nString(
                                    updated.nameShort(), JoreLocaleUtil.SWEDISH);
                    softAssertions
                            .assertThat(actualSwedishNameShort)
                            .overridingErrorMessage(
                                    "Expected the Swedish short name to be: %s but was: %s",
                    ...
                    final String actualFinnishOrigin =
                            JoreLocaleUtil.getI18nString(updated.origin(), JoreLocaleUtil.FINNISH);
                    softAssertions
                            .assertThat(actualFinnishOrigin)
                            .overridingErrorMessage(
                                    "Expected the Finnish origin to be: %s but was: %s",
                    ...
                    final String actualSwedishOrigin =
                            JoreLocaleUtil.getI18nString(updated.origin(), JoreLocaleUtil.SWEDISH);
                    softAssertions
                            .assertThat(actualSwedishOrigin)
                            .overridingErrorMessage(
                                    "Expected the Swedish origin to be: %s but was: %s",
                    ...
                    final String actualFinnishDestination =
                            JoreLocaleUtil.getI18nString(
                                    updated.destination(), JoreLocaleUtil.FINNISH);
                    softAssertions
                            .assertThat(actualFinnishDestination)
                            .overridingErrorMessage(
                                    "Expected the Finnish destination to be: %s but was: %s",
                    ...
                    final String actualSwedishDestination =
                            JoreLocaleUtil.getI18nString(
                                    updated.destination(), JoreLocaleUtil.SWEDISH);
                    softAssertions
                            .assertThat(actualSwedishDestination)
                            .overridingErrorMessage(
                                    "Expected the Swedish destination to be: %s but was: %s",
    ...
    class SetJourneyPatternJore4Ids {
        ...
        private final UUID ROUTE_DIRECTION_ID =
                UUID.fromString("6f93fa6b-8a19-4b98-bd84-b8409e670c70");
        private final UUID JOURNEY_PATTERN_JORE4_ID =
                UUID.fromString("b97185ef-0c27-4548-ac38-3d2cdd0023ac");

        @Nested
        @DisplayName("When the route direction isn't found")
        class WhenRouteDirectionIsNotFound {

            private final UUID UNKNOWN_ROUTE_DIRECTION_ID =
                    UUID.fromString("038174b2-985f-47f7-9676-83cf0b944d88");
            private final List<PersistableJourneyPatternIdMapping> INPUT =
                    List.of(
                            PersistableJourneyPatternIdMapping.of(
                                    UNKNOWN_ROUTE_DIRECTION_ID, JOURNEY_PATTERN_JORE4_ID));
            ...
            void shouldNotUpdateJourneyPatternIdOfExistingRouteDirection() {
                ...
                final RouteDirection routeDirection =
                        targetRepository.findById(RouteDirectionPK.of(ROUTE_DIRECTION_ID)).get();
        ...
        class WhenRouteDirectionIsFound {
            ...
            private final List<PersistableJourneyPatternIdMapping> INPUT =
                    List.of(
                            PersistableJourneyPatternIdMapping.of(
                                    ROUTE_DIRECTION_ID, JOURNEY_PATTERN_JORE4_ID));
            ...
            void shouldUpdateJourneyPatternIdOfExistingRouteDirection() {
                ...
                final RouteDirection routeDirection =
                        targetRepository.findById(RouteDirectionPK.of(ROUTE_DIRECTION_ID)).get();
                assertThat(routeDirection.journeyPatternJore4Id())
                        .contains(JOURNEY_PATTERN_JORE4_ID);
            }
        }
    }
    ...
    class SetRouteJore4Ids {
        ...
        class WhenRouteDirectionIsNotFound {
            ...
            private final UUID UNKNOWN_ROUTE_DIRECTION_ID =
                    UUID.fromString("038174b2-985f-47f7-9676-83cf0b944d88");
            private List<PersistableRouteIdMapping> INPUT =
                    List.of(PersistableRouteIdMapping.of(UNKNOWN_ROUTE_DIRECTION_ID, JORE4_ID));
        ...
        class WhenRouteDirectionIsFound {
            ...
            private List<PersistableRouteIdMapping> INPUT =
                    List.of(PersistableRouteIdMapping.of(ROUTE_DIRECTION_ID, JORE4_ID));
