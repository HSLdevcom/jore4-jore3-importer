...
/**
 * Provides methods which are used to query validity period start and end timestamps from the Jore 4 database. Note that
 * this repository class should only be used by integration tests.
 */
public class Jore4ValidityPeriodTestRepository {
     ...
     */
    public Jore4ValidityPeriodTestRepository(final DataSource dataSource, final ValidityPeriodTargetTable targetTable) {
        this.jdbcTemplate = new JdbcTemplate(dataSource);
        this.sqlQueryGetValidityPeriodEnd =
                String.format(SQL_QUERY_TEMPLATE_GET_VALIDITY_PERIOD_END_TIME, targetTable.getTableName());
        this.sqlQueryGetValidityPeriodStart =
                String.format(SQL_QUERY_TEMPLATE_GET_VALIDITY_PERIOD_START_TIME, targetTable.getTableName());
    }

    /**
     * Finds the validity period start date. Note that this method assumes the target table has exactly one row.
     *
