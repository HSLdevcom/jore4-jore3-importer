...
class LineExportReaderTest {
    ...
    class WhenSourceTablesHaveOneLine {
        ...
        void firstInvocationOfReadMethodMustReturnFoundLine(final SoftAssertions softAssertions) throws Exception {
            ...
            softAssertions
                    .assertThat(line.externalIdOfLineHeader().value())
                    .as("externalIdOfLineHeader")
                    .isEqualTo(EXPECTED_EXTERNAL_ID_OF_LINE_HEADER);

            softAssertions.assertThat(line.lineNumber()).as("lineNumber").isEqualTo(EXPECTED_LINE_NUMBER);

            softAssertions
                    .assertThat(line.validDateRange().range().lowerEndpoint())
                    .as("validDateRangeStartDate")
                    .isEqualTo(EXPECTED_VALID_DATE_RANGE_START);

            softAssertions
                    .assertThat(line.validDateRange().range().lowerBoundType())
                    .as("validDateRangeStartBoundType")
                    .isEqualTo(BoundType.CLOSED);

            softAssertions
                    .assertThat(line.validDateRange().range().upperEndpoint())
                    .as("validDateRangeEndDate")
                    .isEqualTo(EXPECTED_VALID_DATE_RANGE_END);

            softAssertions
                    .assertThat(line.validDateRange().range().upperBoundType())
            ...
            softAssertions.assertThat(finnishShortName).as("Finnish short name").isEqualTo(EXPECTED_FINNISH_SHORT_NAME);

            final String swedishShortName = JoreLocaleUtil.getI18nString(line.shortName(), JoreLocaleUtil.SWEDISH);
            softAssertions.assertThat(swedishShortName).as("Swedish short name").isEqualTo(EXPECTED_SWEDISH_SHORT_NAME);

            softAssertions.assertThat(line.networkType()).as("Network type").isEqualTo(EXPECTED_NETWORK_TYPE);

            softAssertions.assertThat(line.typeOfLine()).as("Type of line").isEqualTo(EXPECTED_TYPE_OF_LINE);
