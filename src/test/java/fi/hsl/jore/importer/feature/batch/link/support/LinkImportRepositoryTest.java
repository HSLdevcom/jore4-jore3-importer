...
public class LinkImportRepositoryTest extends IntegrationTest {
    ...
    public void whenNewStagedRowsAndCommit_andTargetDbContainsNodes_thenReturnResultWithInsertedIdAndNodeRefs() {
        ...
        final NodePK startNodePk =
                nodeRepository.insert(PersistableNode.of(startNodeExtId, NodeType.CROSSROADS, POINT_1));
        final NodeId endNodeId = NodeId.of("2");
        final ExternalId endNodeExtId = ExternalIdUtil.forNode(endNodeId);
        final NodePK endNodePk = nodeRepository.insert(PersistableNode.of(endNodeExtId, NodeType.CROSSROADS, POINT_2));
    ...
    public void whenStagedRowsWithChangesAndCommit_andTargetNotEmpty_thenReturnResultWithUpdatedId() {
        ...
        final LinkPK existingId =
                targetRepository.insert(PersistableLink.of(externalId, networkType, LINE_1, startNodePk, endNodePk));
        ...
        assertThat("The updated rows location was changed", geometriesMatch(link.geometry(), LINE_2), is(true));
        assertThat(link.startNode(), is(startNodePk));
        assertThat(link.endNode(), is(endNodePk));
    }
    ...
    public void whenStagedRowsWithNoChangesAndCommit_andTargetNotEmpty_thenReturnEmptyResult() {
        ...
        assertThat("The target row was not changed", geometriesMatch(link.geometry(), LINE_1), is(true));
        assertThat(link.startNode(), is(startNodePk));
        assertThat(link.endNode(), is(endNodePk));
    }
    ...
    public void whenStagedRowsWithExternalIdChangesAndCommit_andTargetNotEmpty_thenReturnDeleteAndInsertResult() {
        ...
        final ExternalId newExternalId = ExternalIdUtil.forLink(newTransitType, startNodeId, endNodeId);
        importRepository.submitToStaging(List.of(
                Jore3Link.of(newExternalId, newNetworkType, sourceLink.geometry(), startNodeExtId, endNodeExtId)));
        ...
        assertThat("A single link exists in the target repository", targetRepository.count(), is(1));

        final Link modifiedLink = targetRepository.findAll().get(0);

        assertThat("The target row was not changed", geometriesMatch(modifiedLink.geometry(), LINE_1), is(true));
    ...
    public void whenStagedRowsAndCommit_andTargetContainsExtraRows_thenReturnResultWithDeletedId() {
        ...
        final TransitType transitType = TransitType.BUS;
        final NetworkType networkType = NetworkType.ROAD;
        final ExternalId firstLinkExternalId = ExternalIdUtil.forLink(transitType, firstNodeId, secondNodeId);
        final ExternalId secondLinkExternalId = ExternalIdUtil.forLink(transitType, thirdNodeId, fourthNodeId);
        final PersistableLink firstLink =
                PersistableLink.of(firstLinkExternalId, networkType, LINE_1, nodeIds.get(0), nodeIds.get(1));
        final PersistableLink secondLink =
                PersistableLink.of(secondLinkExternalId, networkType, LINE_2, nodeIds.get(2), nodeIds.get(3));
