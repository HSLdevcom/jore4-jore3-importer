...
/**
 * Provides methods which allows us to write assertions
 * for the validity period of entities exported to Jore 4 database.
 */
public class ValidityPeriodAssertions {
    ...
    /**
     * Ensures that the validity period starts at the given date AND the given
     * date is included in the validity period.
    ...
    public ValidityPeriodAssertions assertThatValidityPeriodStartsAt(final LocalDate expectedStartDate) {
        softAssertions.assertThat(validityPeriod.range().lowerEndpoint())
                .as("validityPeriodStartDate")
                .isEqualTo(expectedStartDate);

        softAssertions.assertThat(validityPeriod.range().lowerBoundType())
    ...
    /**
     * Ensures that the validity period ends at the given date AND the given
     * date isn't included in the validity period.
    ...
    public ValidityPeriodAssertions assertThatValidityPeriodEndsAt(final LocalDate expectedEndDate) {
        softAssertions.assertThat(validityPeriod.range().upperEndpoint())
                .as("validityPeriodEndDate")
                .isEqualTo(expectedEndDate);

        softAssertions.assertThat(validityPeriod.range().upperBoundType())
